#!/usr/bin/env python
"""
Génère et trace les propriétés du gaz de pyrolyse du bois de chêne à
l'équilibre chimique : enthalpie h_g, masse molaire M, Cp, gamma, masse
volumique rho et viscosité mu, en fonction de la température, pour plusieurs
pressions. Utilise l'outil MutationPP `mppequil` comme moteur de calcul.
...
Composition, issue de la fermeture élémentaire bois sec - char
(cf. oak_pyrolysis_data.py), rendement en char 20 % :
    oak_pyro : C:0.221, H:0.536, O:0.243 (fractions molaires élémentaires)

Hypothèse : équilibre chimique du gaz, sans phase condensée. Le mélange
`oak-pyrogas` reprend la liste d'espèces C/H/O de `oak-air` privée de C(gr)
— le gaz de pyrolyse pur, avant qu'il n'atteigne la paroi, n'est ni mélangé à
l'air ni en contact avec le char.

Ce gaz est pauvre en carbone et riche en oxygène (C:0.221, O:0.243) : il est
loin de la limite de dépôt de suie, la convention « sans phase condensée »
...
Usage :
    python oak_pyrolysis_gas.py

Prérequis :
    - Le binaire `mppequil` doit être dans le PATH ou dans build/src/apps/
    - Le fichier data/mixtures/oak-pyrogas.xml doit exister
...
def plot_gas_properties(all_data, pressures_atm):
    ...
    fig.suptitle(
        "Propriétés du gaz de pyrolyse du liège/phénolique à l'équilibre  "
...
def plot_enthalpy_focus(all_data, pressures_atm):
    ...
    fig.suptitle(
        r"Enthalpie du gaz de pyrolyse du liège/phénolique à l'équilibre  $h_g(T, P)$",
        fontsize=13)
    ...
    plt.tight_layout()
    out_png = "oak_pyrolysis_gas_enthalpy.png"
    plt.savefig(out_png, dpi=150)
    print(f"Figure sauvegardée : {out_png}")
    plt.close()
