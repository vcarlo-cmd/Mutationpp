#!/usr/bin/env python
"""
Génère et trace la table B' d'un bois de chêne dans l'air.

Matériau :
    - bois de chêne massif (Quercus robur / Q. petraea), SEC, sans résine
    - analyse élémentaire C 50.0 / H 6.1 / O 43.9 % masse (littérature,
      provenance à confirmer ; unité de répétition globale C6H8.7O3.95)
    - rendement en char : 20 % (donnée de l'énoncé)

    Un seul constituant, qui pyrolyse entièrement : pas de renfort inerte
    (TACOT, CPh70), pas de mélange de deux gaz (chêne).

    Sur 100 g de bois sec : 20 g de char + 80 g de gaz
    ...
    un volume constant, que le bois ne respecte pas (il se rétracte en
    carbonisant).

Composition du gaz de pyrolyse (fermeture élémentaire bois - char) :
    C:0.221, H:0.536, O:0.243 (fractions molaires élémentaires)
Char : carbone pur, C:1.0

Voir oak_pyrolysis_data.py pour le calcul de ces compositions et
mise_en_donnees_chene.md pour la mise en données complète.

Le mélange oak-air reprend la liste de 25 espèces de la table B' de
...
Sorties :
    ...
    - B'c en fonction de B'g à température fixée, avec le point de
      fonctionnement stationnaire du chêne et celui du TACOT
    - la table du point de fonctionnement stationnaire

Usage :
    python oak_bprime.py

Prérequis :
    - binaire `bprime` dans le PATH ou dans build/src/apps/
    - data/mixtures/oak-air.xml
    - numpy, matplotlib
"""
...
# Le chêne fonctionne à fort B'g : on pousse le balayage à 5.
...
K_OAK   = 4.000       # m_gaz/m_char = 80 g gaz / 20 g char
...
# rho_v : chêne sec, ordre de grandeur. rho_c : NON MESURÉE -- hypothèse
# V_char/V_vierge = 0.55 (le bois se rétracte), à remplacer par une mesure.
RHO_VIRGIN = 700.0
RHO_CHAR   = RHO_VIRGIN * 0.20 / 0.55
...
def plot_isobar_table(all_data, pressures_atm, bg, out_png):
    ...
    fig.suptitle(
        rf"Table B' — chêne dans l'air  ({bg_label(bg)},  "
...
def plot_bg_comparison(results_by_bg):
    ...
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(14, 6))
    fig.suptitle(
        r"Influence de $B'_g$ sur la table B' — chêne dans l'air (P = 1 atm)",
        fontsize=13,
    )
    ...
    plt.tight_layout()
    out_png = "oak_bprime_bg_comparison.png"
    plt.savefig(out_png, dpi=150)
...
def plot_bc_vs_bg(sweep, out_png):
    ...
    La courbe tracée est celle du chêne. Les deux jeux de
    symboles montrent où la MEME courbe est lue selon le couplage matériau :
    k = 4.0 (chêne, étoiles) ou k = 0.2727 (couplage typique
    ...
    fig.suptitle(
        r"$B'_c$ en fonction de $B'_g$ — chêne ; "
    ...
    for ax, P in zip(axes, SWEEP_PRESSURES):
        ...
        ax.plot(bgline, bgline / K_OAK, "k--", lw=1.2, alpha=0.7,
                label=rf"chêne : $B'_g = {K_OAK:g}\,B'_c$  ($\star$)")
...
def steady_state_table(bprime_path, out_csv):
    """
    Table du point de fonctionnement stationnaire du chêne :
    ...
    with open(out_csv, "w", newline="") as f:
        ...
        for P in SWEEP_PRESSURES:
            ...
            for j, T in enumerate(Ts):
                bcs = np.array([raw[(P, bg)][j, 1] for bg in BG_SWEEP])
                hws = np.array([raw[(P, bg)][j, 2] for bg in BG_SWEEP])
                bc = solve_operating_point(np.array(BG_SWEEP), bcs, K_OAK)
                bg = min(K_OAK * bc, BG_SWEEP[-1])
...
if __name__ == "__main__":
    ...
    for bg in BG_VALUES:
        ...
        out_csv = f"oak_bprime_Bg{bg_filename(bg)}.csv"
        with open(out_csv, "w", newline="") as f:
            writer = csv.writer(f)
        ...
        plot_isobar_table(all_data, PRESSURES_ATM, bg,
                          f"oak_bprime_Bg{bg_filename(bg)}.png")
    ...
    out_csv = "oak_bprime_bc_table.csv"
    with open(out_csv, "w", newline="") as f:
        w = csv.writer(f)
    ...
    # (nTw x nP lignes)
    out_csv = "oak_bprime_hw_table.csv"
    with open(out_csv, "w", newline="") as f:
        w = csv.writer(f)
    ...
    out_csv = "oak_bprime_Bc_vs_Bg.csv"
    with open(out_csv, "w", newline="") as f:
        w = csv.writer(f)
    ...
    plot_bc_vs_bg(sweep, "oak_bprime_Bc_vs_Bg.png")

    # --- Point de fonctionnement ------------------------------------------
    print()
    steady_state_table(bprime_path, "oak_bprime_steady_state.csv")

    # --- Récapitulatif -----------------------------------------------------
    print("\n" + "=" * 78)
    print("POINTS DE FONCTIONNEMENT — chêne (k = 4.0) vs TACOT (k = 0.2727)")
    print("=" * 78)
    print(f"{'P atm':>7} {'T K':>6} | {'chene Bc':>10} {'chene Bg':>9} | "
          f"{'k=0.273 Bc':>11} {'Bg':>9} | {'ecart Bc':>9}")
    for (P, T), (bgs, bcs, _) in sorted(sweep.items()):
        a = solve_operating_point(bgs, bcs, K_OAK)
        b = solve_operating_point(bgs, bcs, K_TACOT)
        rel = (a - b) / max(b, 1e-12) * 100.0
        print(f"{P:>7g} {T:>6g} | {a:>10.5f} {K_OAK*a:>9.4f} | "
