...
| mélange | `data/mixtures/cp70-air_25.xml` (25 espèces + C(gr)) |
| gaz de pyrolyse | C:0.206, H:0.679, O:0.115 |
| char | C:1.0 |

```bash
cd cp70_bprime && python cp70_bprime.py
```
...
Pour un même triplet (T, P, B'g), CP70 et TACOT donnent exactement le même B'c
...
soit **k = 0.1385 pour le CP70** contre **0.2727 pour le TACOT**. Le point de
fonctionnement est l'intersection de la courbe B'c(B'g) — commune — avec cette
droite, propre à chaque matériau.

C'est exactement ce que montre `cp70_bprime_Bc_vs_Bg.png` : les courbes colorées
sont communes, les ★ (CP70) et ■ (TACOT) sont les intersections.

### Points de fonctionnement

| P [atm] | T [K] | CP70 B'c | CP70 B'g | TACOT B'c | TACOT B'g | écart B'c |
|---------|-------|----------|----------|-----------|-----------|-----------|
...
> `ṡ = B'c·ṁe/ρc`. Le CP70 étant 5.85 fois plus dense en char, il recule
> d'autant moins vite. La colonne `recession_over_mdote_m3_per_kg` de
> `cp70_bprime_steady_state.csv` donne directement `B'c/ρc`.
...
| Fichier | Contenu |
|---------|---------|
| `cp70_bprime_Bg{0p0,0p1,0p2,0p5,1p0,2p0}.csv` | table B' complète : 25 isobares (10⁻³ → 10³ atm) × 189 T (300–5000 K, pas 25 K) × 26 fractions molaires |
| `cp70_bprime_Bg*.png` | isobares B'c et h_w pour chaque B'g |
| `cp70_bprime_bg_comparison.png` | influence de B'g à 1 atm |
| `cp70_bprime_Bc_vs_Bg.csv` / `.png` | **B'c en fonction de B'g** à T fixée, avec les points de fonctionnement |
| `cp70_bprime_steady_state.csv` | point de fonctionnement stationnaire : `Bc_ss`, `Bg_ss`, `hw_ss`, plus `Bc_Bg0` (référence sans pyrolyse) et `B'c/ρc` |
...
`cp70_bprime_steady_state.csv` (B'g_ss = 10) sont des **artefacts numériques**,
