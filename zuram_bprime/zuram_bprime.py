#!/usr/bin/env python
"""
Génère et trace la table B'c / h_w du Zuram (composite carbone/phénolique VKI)
dans l'air.
Utilise le binaire C++ MutationPP `bprime` comme moteur de calcul.

Mise en données, reprise telle quelle du cas de référence AblaNTIS
`carbonPhenolInAir` :
...
Le mélange `zuram-air` reproduit exactement la liste de 20 espèces gazeuses de
la table B' de référence (+ C(gr) pour la phase condensée). Voir
zuram_validation.py pour la comparaison quantitative à cette table.
...
5000 K, pour six valeurs de B'g : 0, 0.1, 0.2, 0.5, 1.0, 2.0.

Attention au plafond numérique : `surfaceMassBalance` ajoute une quantité finie
de char, max(100*B'g, 200). Quand le char est entièrement sublimé, B'c sature
exactement sur cette valeur — ce n'est pas une solution physique mais la
signature de B'c -> l'infini. Ces points sont repérés dans la colonne `regime`
du CSV et masqués sur les graphiques.

Usage :
    python zuram_bprime.py

Prérequis :
    - Le binaire `bprime` doit être dans le PATH ou dans build/src/apps/
    - Le fichier data/mixtures/zuram-air.xml doit exister
...
def char_cap(bg):
    """
    Plafond numérique de B'c : quantité de char ajoutée par
    `Thermodynamics::surfaceMassBalance` (max(100*B'g, 200)).
    B'c = cap signifie « tout le char ajouté est passé en phase gazeuse »,
    c.-à-d. B'c infini physiquement.
    """
...
def masked_bc(Bc, bg):
    """B'c avec les points saturés (plafond de char) remplacés par NaN."""
    out = np.array(Bc, dtype=float)
    out[out >= char_cap(bg) * (1.0 - 1e-9)] = np.nan
    return out
...
def plot_isobar_table(all_data, pressures_atm, bg, out_png):
    """
    Trace la table B' classique (isobares) pour un B'g donné.
    ...
    fig.suptitle(
        rf"Table B' — Zuram dans l'air  ({bg_label(bg)},  "
...
def plot_bg_comparison(results_by_bg):
    ...
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(14, 6))
    fig.suptitle(
        r"Influence de $B'_g$ sur la table B' — Zuram dans l'air (P = 1 atm)",
        fontsize=13,
    )
    ...
    plt.tight_layout()
    out_png = "zuram_bprime_bg_comparison.png"
    plt.savefig(out_png, dpi=150)
...
if __name__ == "__main__":
    ...
    for bg in BG_VALUES:
        ...
        # Sauvegarde CSV
        out_csv = f"zuram_bprime_Bg{bg_filename(bg)}.csv"
        n_sat = 0
        with open(out_csv, "w", newline="") as f:
            writer = csv.writer(f)
            writer.writerow(["Bg", "P_atm", "regime"] + header)
            for P_atm, (_, data) in zip(PRESSURES_ATM, all_data):
                for row in data:
                    saturated = row[1] >= cap * (1.0 - 1e-9)
                    n_sat += saturated
                    writer.writerow(
                        [f"{bg}", f"{P_atm:.6g}",
                         "SATURE" if saturated else "OK"]
                        + [f"{v:.6e}" for v in row]
                    )
        print(f"  Table CSV sauvegardée : {out_csv}"
              f"  ({n_sat} points saturés sur le plafond B'c = {cap:g})")

        # Graphique isobares pour ce B'g
        out_png = f"zuram_bprime_Bg{bg_filename(bg)}.png"
    ...
    out_csv = "zuram_bprime_bc_table.csv"
    with open(out_csv, "w", newline="") as f:
        w = csv.writer(f)
    ...
    out_csv = "zuram_bprime_hw_table.csv"
    with open(out_csv, "w", newline="") as f:
        w = csv.writer(f)
