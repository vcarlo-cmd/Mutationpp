#!/usr/bin/env python
"""
Génère et trace la table B'c du liège P50 dans l'air.
Utilise le binaire C++ MutationPP `bprime` comme moteur de calcul.

Composition du liège P50 (fractions massiques) :
    45% subérine (C₁₈H₃₄O₄), 27% lignine, 12% polysaccharides (C₆H₁₀O₅),
    6% céroides (C₃₀H₅₀O₂), 6% tanins (C₁₅H₁₄O₆)
    → matériau vierge : C=65.1%, H=8.5%, O=26.4% (masse)

Gaz de pyrolyse (rendement char = 22%, char = C pur) :
    C : (65.1-22)/12 = 3.59 mol
    H : 8.5/1        = 8.50 mol
    O : 26.4/16      = 1.65 mol
    → ratios molaires cork_pyro : C:0.50, H:1.18, O:0.23
...
Prérequis :
    ...
    - Le fichier data/mixtures/cork-air.xml doit exister
...
def plot_isobar_table(all_data, pressures_atm, bg, out_png):
    ...
    fig.suptitle(
        rf"Table B' — Liège P50 dans l'air  ({bg_label(bg)},  "
...
def plot_bg_comparison(results_by_bg):
    ...
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(14, 6))
    fig.suptitle(
        r"Influence de $B'_g$ sur la table B' — Liège P50 dans l'air (P = 1 atm)",
        fontsize=13,
    )
...
if __name__ == "__main__":
    ...
    for bg in BG_VALUES:
        ...
        out_png = f"cork_bprime_Bg{bg_filename(bg)}.png"
