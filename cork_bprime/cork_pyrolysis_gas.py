#!/usr/bin/env python
"""
Génère et trace les propriétés du gaz de pyrolyse du liège/phénolique à
...
(cf. cork_pyrolysis_data.py) :
    cork_pyro : C:0.240, H:0.631, O:0.129 (fractions molaires élémentaires)

Elle mélange le gaz du liège (C:0.235, H:0.632, O:0.133, 60 g sur 100 g de
vierge) et celui de la résine (C:0.269, H:0.626, O:0.104, 10 g).

Hypothèse : équilibre chimique du gaz, sans phase condensée. Le mélange
`cork-pyrogas` reprend la liste d'espèces C/H/O de `cork-air` privée de C(gr)
...
(C:0.240 contre C:0.206 pour le TACOT) : la convention « sans phase
...
Usage :
    python cork_pyrolysis_gas.py

Prérequis :
    - Le binaire `mppequil` doit être dans le PATH ou dans build/src/apps/
    - Le fichier data/mixtures/cork-pyrogas.xml doit exister
...
def plot_gas_properties(all_data, pressures_atm):
    ...
    plt.tight_layout()
    out_png = "cork_pyrolysis_gas.png"
...
def plot_enthalpy_focus(all_data, pressures_atm):
    ...
    plt.tight_layout()
    out_png = "cork_pyrolysis_gas_enthalpy.png"
    plt.savefig(out_png, dpi=150)
    print(f"Figure sauvegardée : {out_png}")
    plt.close()
...
if __name__ == "__main__":
    ...
    out_csv = "cork_pyrolysis_gas.csv"
