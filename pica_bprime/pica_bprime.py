#!/usr/bin/env python
"""
Génère et trace la table B'c du PICA (Phenolic Impregnated Carbon Ablator) dans l'air.
Utilise le binaire C++ MutationPP `bprime` comme moteur de calcul.

Composition du gaz de pyrolyse (Goldstein 1992) :
    CH4:0.13, CO:0.23, CO2:0.11, H2:0.12, H2O:0.39, C2H4:0.02 (fractions molaires)
...
Usage :
    python pica_bprime.py

Prérequis :
    - Le binaire `bprime` doit être dans le PATH ou dans build/src/apps/
    - Le fichier data/mixtures/pica-air.xml doit exister
...
def plot_isobar_table(all_data, pressures_atm, bg, out_png):
    ...
    fig.suptitle(
        rf"Table B' — PICA dans l'air  ({bg_label(bg)},  "
...
def plot_bg_comparison(results_by_bg):
    ...
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(14, 6))
    fig.suptitle(
        r"Influence de $B'_g$ sur la table B' — PICA dans l'air (P = 1 atm)",
        fontsize=13,
    )

    for idx, bg in enumerate(BG_VALUES):
        pressures_atm, all_data = results_by_bg[bg]
        # Trouver la pression la plus proche de 1 atm
        closest = min(pressures_atm, key=lambda p: abs(np.log10(p)))
        _, data = min(
    ...
    plt.tight_layout()
    out_png = "pica_bprime_bg_comparison.png"
    plt.savefig(out_png, dpi=150)
...
if __name__ == "__main__":
    ...
    for bg in BG_VALUES:
        ...
        # Sauvegarde CSV
        out_csv = f"pica_bprime_Bg{bg_filename(bg)}.csv"
        ...
        out_png = f"pica_bprime_Bg{bg_filename(bg)}.png"
